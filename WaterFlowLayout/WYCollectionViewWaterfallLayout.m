//
//  WYCollectionViewWaterfallLayout.m
//  PhotoTool
//
...
@property (nonatomic, weak) id <WYCollectionViewDelegateWaterfallLayout> delegate;
...
@implementation WYCollectionViewWaterfallLayout
...
- (void)commonInit {
    ...
    _itemRenderDirection = WYTCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst;
...
- (void)setItemRenderDirection:(WYTCollectionViewWaterfallLayoutItemRenderDirection)itemRenderDirection {
...
- (void)recalculateItemSize {
    CGFloat width = ([UIScreen mainScreen].bounds.size.width - self.sectionInset.left - self.sectionInset.right - (_columnCount - 1) * self.minimumInteritemSpacing) / _columnCount;
    self.itemSize  = CGSizeMake(CHTFloorCGFloat(width), CHTFloorCGFloat(width));
}
...
- (id <WYCollectionViewDelegateWaterfallLayout> )delegate {
    return (id <WYCollectionViewDelegateWaterfallLayout> )self.collectionView.delegate;
}
...
- (void)prepareLayout {
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        CGFloat width = self.collectionView.bounds.size.width - sectionInset.left - sectionInset.right;
        NSInteger columnCount = [self columnCountForSection:section];
        CGFloat columnWidth = CHTFloorCGFloat((width - (columnCount - 1 + self.firstColumnMultiply) * columnSpacing) / (columnCount + self.firstColumnMultiply));
        ...
        CGFloat itemWidth = columnWidth;
        for (idx = 0; idx < itemCount; idx++) {
            if (idx == 0 && self.firstColumnMultiply > 0) {
                itemWidth = columnWidth * (self.firstColumnMultiply + 1) + columnSpacing * self.firstColumnMultiply;
            } else {
            ...
            if (idx != 0 && self.firstColumnMultiply > 0) {
                xOffset += columnWidth * self.firstColumnMultiply + columnSpacing * self.firstColumnMultiply;
            }
        ...
        top = [self.columnHeights[section][columnIndex] floatValue] - minimumLineSpacing + sectionInset.bottom;
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
    ...
    switch (self.itemRenderDirection) {
        case WYTCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst:
            index = [self shortestColumnIndexInSection:section];
            break;
            
        case WYTCollectionViewWaterfallLayoutItemRenderDirectionLeftToRight:
            index = (item % columnCount);
            break;
            
        case WYTCollectionViewWaterfallLayoutItemRenderDirectionRightToLeft:
